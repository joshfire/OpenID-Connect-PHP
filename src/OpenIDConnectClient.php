...
class OpenIDConnectClient
{
    ...
    public function signOut($accessToken, $redirect)
    {
        ...
        $signout_endpoint .= (false === strpos($signout_endpoint, '?') ? '?' : '&').http_build_query($signout_params, null, '&');
    ...
    {
        if (false !== parse_url($url, PHP_URL_HOST)) {
    ...
    public function getRedirectURL()
    {
        ...
        if (isset($_SERVER['HTTP_UPGRADE_INSECURE_REQUESTS']) && (1 == $_SERVER['HTTP_UPGRADE_INSECURE_REQUESTS'])) {
            $protocol = 'https';
        } else {
    ...
    private function requestAuthorization()
    {
        ...
        $auth_endpoint .= (false === strpos($auth_endpoint, '?') ? '?' : '&').http_build_query($auth_params, null, '&');
    ...
    private function verifyJWTclaims($claims, $accessToken = null)
    {
        if (isset($claims->at_hash) && isset($accessToken)) {
            if (isset($this->getAccessTokenHeader()->alg) && 'none' != $this->getAccessTokenHeader()->alg) {
    ...
    public function getVerifiedClaims($attribute = null)
    {
        ...
        } elseif (array_key_exists($attribute, $this->verifiedClaims)) {
